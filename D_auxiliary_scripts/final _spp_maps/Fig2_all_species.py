...
overwrite_res=1
df_to_exclude=["inset"]
single_frame_with_zoom=True
extent_from_lyr_n=0 #0 for first loaded layer
        ...
        for df in arcpy.mapping.ListDataFrames(mxd):
            if df.name not in df_to_exclude:
                ...
                for i in range(len(To_load)):
                    ...
                    if single_frame_with_zoom and i==extent_from_lyr_n:
                        #MUST ZOOM TO AREA WHERE SPECIES OCCUR!
                        raster1=arcpy.Raster(fc)
                        extent = raster1.extent
                        df.extent = extent
...
tifdir=r"Y:/PICCC_analysis/FB_analysis/model_results/biomod2finalmodel_P_PA_oldcode_less_PAs/output_rasters/main/"
refdir="D:/Dropbox/current work/FB SDM/FB MS draft/not in MS/base figs/140807 version/indiv_sp_map/FB_fig2_ref_layrs/"
resultsDir="D:/Dropbox/current work/FB SDM/FB MS draft/not in MS/base figs/140807 version/indiv_sp_map/FB_fig2/"
spp=['Akekee', 'Akiapolauu', 'Akikiki', 'Akohekohe', 'Anianiau', 'Hawaii_Akepa', 'Hawaii_Creeper', 'Oahu_Amakihi','Hawaii_Elepaio', 'Kauai_Elepaio', 'Maui_Alauahio', 'Maui_Parrotbill', 'Omao', 'Oahu_Elepaio', 'Palila', 'Puaiohi', 'Kauai_Amakihi', 'Hawaii_Amakihi', 'Apapane', 'Iiwi']
#spp=['Akekee', 'Akiapolauu', 'Akikiki', 'Anianiau', 'Hawaii_Akepa', 'Hawaii_Creeper', 'Hawaii_Elepaio', 'Kauai_Elepaio','Omao', 'Palila', 'Puaiohi', 'Kauai_Amakihi']
for sp in spp:
    #sp="Apapane"
    To_load=[tifdir+"%s_response_zones_ROC_ef.pmw_int.tif"%(sp), shpdir+"%s_current_veg_mask.shp"%(sp)] #locs
    ref_layers=[refdir+"FB_response_zones3.lyr",refdir+"Current_habitat.lyr"] #locs
    names_to_load=["response_zones", "primary_habitat"] #strings for referencing
    output_path=resultsDir+sp+"_fig2_single_frame.tif" #_no_legnd (by temporarily deleting legend and rerunning)
