...
spp_nm=c("Akikiki", "Akekee")#, "Kauai_Amakihi", "Oahu_Amakihi")#, "Apapane")# "Akekeke", "Akikiki", "Anianiau", "Kauai_Amakihi", "Kauai_Elepaio", "Puaiohi", "Oahu_Amakihi", "Oahu_Elepaio", "Apapane", "Iiwi",)   #"Akekee", "Akikiki", "Anianiau", "Apapane", "Iiwi", "Kauai_Amakihi", "Kauai_Elepaio", "Oahu_Amakihi", "Oahu_Elepaio", "Puaiohi"
server=1

if (server==1){
  working_dir='Y:/FB_analysis/FB_SDM/biomod2/multi_core_test/'
...
MyBiomodSF <- function(sp_nm){
  ...
  if (file.exists(FileName00)==F | overwrite==1){    
    ...
    jnk=round(runif(1)*10000)
    tmp_workspace=paste(jnk,"_temp_workspace1.RData", sep="")
    save.image(tmp_workspace)   #to save workspace
    ...
    load(tmp_workspace)
    unlink(tmp_workspace, recursive = FALSE, force = FALSE)
...
## Do the run
mySFModelsOut <- sfLapply( spp_nm, MyBiomodSF)
## stop snowfall
sfStop( nostop=FALSE )

#####non parallel
#myLapply_SFModelsOut <- lapply( sp_nm, MyBiomodSF)