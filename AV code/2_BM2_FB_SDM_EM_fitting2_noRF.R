...
#spp_nm=c("Hawaii_Elepaio", "Palila", "Oahu_Elepaio", "Kauai_Elepaio", "Kauai_Amakihi", "Oahu_Amakihi", "Amakihi", "Elepaio", "Akohekohe")
...
clim_data_dir00="D:/GIS_Data/REnviroLayers/mixed_data_2000_250m/"
...
for (sp_nm in spp_nm){
  ...
  if (file.exists(workspace_name_out)==F | overwrite==1){
    ...
    myBiomodEM <- BIOMOD_EnsembleModeling(
      ...
      eval.metric = c('TSS', 'ROC', 'KAPPA'),#; 'all', #c('TSS', 'ROC'),
      eval.metric.quality.threshold = c(0.15, 0.15, 0.15),
      prob.mean = T,
      prob.cv = T,
      prob.ci = T,
      prob.ci.alpha = 0.05,
