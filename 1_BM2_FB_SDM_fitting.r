...
sp_parallel_run=function(sp_nm){
  ...
  if (file.exists(FileName00) == F | overwriteData == T){ #check if analysis for species already done or overwrite requested in config    
    ...
    myBiomodModelEval <- get_evaluations(myBiomodModelOut) #creates an array with model evaluation results for all models   
    ...
    Spp_VariImp <- data.frame(get_variables_importance(myBiomodModelOut)) #creates data frame with model variable importances
