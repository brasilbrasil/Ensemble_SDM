...
#spp_nm=c("Akikiki")#, "Kauai_Elepaio", "Oahu_Amakihi", "Oahu_Elepaio", "Puaiohi")#, "Akikiki", "Anianiau", "Apapane", "Iiwi", "Kauai_Amakihi", "Kauai_Elepaio", "Oahu_Amakihi", "Oahu_Elepaio", "Puaiohi")   #"Akekee", "Akikiki", "Anianiau", "Apapane", "Iiwi", "Kauai_Amakihi", "Kauai_Elepaio", "Oahu_Amakihi", "Oahu_Elepaio", "Puaiohi"
server=1

models_to_run=c('GBM','RF','MAXENT')
if (server==1){
  working_dir='Y:/FB SDM/biomod2/additional_runs_1_2_3_7_12_15/'
  ...
  clim_data_dir0="C:/Users/lfortini/Data/temp_data/FB_yearly_clim_data/yearly_clim_data_2000_100m/"
}

dirs=list.dirs(data_dir, full.names = FALSE, recursive = TRUE)
for (dir in dirs){
  layers<-list.files(dir, pattern='*', full.names=FALSE)
  for (layer in layers){
    layer_full_nm=paste(dir,layer, sep="/")
    cat('\n','found ', layer, 'in ', dir)
    out_dir_nm=str_replace(dir, data_dir, working_dir)
    dir.create(out_dir_nm, showWarnings = FALSE, recursive = TRUE, mode = "0777")
    out_lyr_nm=str_replace(layer_full_nm, data_dir, working_dir)
    #out_lyr_nm=str_replace(out_lyr_nm, bl_lr, output_filenm)
    if (file.exists(out_lyr_nm)==F | overwrite_past_results==1){
...
env_var_files=c("bio1.grd", "bio2.grd", "bio3.grd", "bio7.grd", "bio12.grd", "bio15.grd") 
...
#     write.table(Spp_KAP, file = FileName, sep=",", col.names=NA)
    ...
    rm(list=c("sp_nm","local_config_dir", "spp_nm", "models_to_run", "working_dir", 
              ...
              "clim_data_2100", "working_dir", "csv_dir", "eval_stats",  "crop_raster"))      
