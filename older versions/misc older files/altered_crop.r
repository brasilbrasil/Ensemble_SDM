altered_crop <- function (x, y, filename = "", snap = "near", datatype = NULL, 
                      ...) 
  {
    ...
    if (row1 == 1 & row2 == nrow(x) & col1 == 1 & col2 == 
      ncol(x)) {
      return(x)
    }
    nc <- ncol(out)
    ...
    if (canProcessInMemory(out, 3)) {
      v <- getValuesBlock(x, row1, nrows = nr, col = col1, 
                          ncols = nc)
      out <- setValues(out, v)
      if (filename != "") {
      ...
      out <- writeStart(out, filename = filename, datatype = datatype, 
                        ...)
      for (i in 1:tr$n) {
        vv <- getValuesBlock(x, row = tr$row[i] + row1 - 
          1, nrows = tr$nrows[i], col1, nc)
