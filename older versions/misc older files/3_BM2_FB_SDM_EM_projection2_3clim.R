...
if (server==1){
  ...
  clim_data_2000="Y:/SDM_env_data/bioclim_variables/FB_clim_var_data/yearly_clim_data_2000_250m/"
  clim_data_2100="Y:/SDM_env_data/bioclim_variables/FB_clim_var_data/yearly_clim_data_2100_250m/"
...
for (sp_nm in spp_nm){
  ...
  if (file.exists(workspace_name_out)==F | overwrite==1){
    ...
    if (file.exists(workspace_name_out0)==F | overwrite==1){  
      ...
      if (plot_graphs==1){
        for (model in models_to_run){
          if (file.exists(paste(proj_nm, "_", sp_nm, "_bin_ROC_RasterStack" , sep = ""))){
