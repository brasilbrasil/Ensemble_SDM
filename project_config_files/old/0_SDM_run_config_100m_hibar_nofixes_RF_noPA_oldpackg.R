...
spp_nm=c("Akekee", "Hawaii_Akepa", "Palila")#, "Kauai_Amakihi", "Hawaii_Akepa", "Palila")
project_name='test_runs_100m_hibar_nofixes_RF_noPA_oldpackg'
server=1
overwrite=0
...
if (EM_fit){
  source(paste0(DR_code_S,"Ensemble_SDM/Em_code_before_PA_correction/1_BM2_FB_SDM_fitting_w_cropping4_oldpackg.r")) #this is where all configurations are at
}
if (EM_ensemble){
...
if (EM_project){
  source(paste0(DR_code_S,"Ensemble_SDM/Em_code_before_PA_correction/3_BM2_FB_SDM_EM_projection_with_crop4_oldpackg.r")) #this is where all configurations are at
}
