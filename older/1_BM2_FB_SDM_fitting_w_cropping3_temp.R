...
spp_nm=c("Hawaii_Akepa")# "Akekeke", "Akikiki", "Anianiau", "Kauai_Amakihi", "Kauai_Elepaio", "Puaiohi", "Oahu_Amakihi", "Oahu_Elepaio", "Apapane", "Iiwi",)   #"Akekee", "Akikiki", "Anianiau", "Apapane", "Iiwi", "Kauai_Amakihi", "Kauai_Elepaio", "Oahu_Amakihi", "Oahu_Elepaio", "Puaiohi"
server=1

models_to_run=c('GBM','RF','MAXENT')
if (server==1){
  working_dir='Y:/FB analysis/FB SDM/biomod2/test_runs_5_6_12/'
...
for (sp_nm in spp_nm){
  ...
  #jnk_pres= XY_pres_extrnoNA[, 'cells'] %in% XYabackg_extrnoNA[, 'cells']  ####DEBUG DEBUG DEBUG
  #jnk_pres_coor= XY_pres_extrnoNA[which(jnk_pres==TRUE),1:4]
  #jnk_pres_coor=jnk_pres_coor[order(jnk_pres_coor[,4]),]
  #jnk_coor=jnk_coor[order(jnk_coor[,4]),]
  #jnk_pres_coor[,2]-jnk_coor[,2]
  #jnk_pres_coor[,1]-jnk_coor[,1]
