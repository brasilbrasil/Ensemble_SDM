...
#spp_nm=c("Hawaii_Amakihi", "Hawaii_Creeper")#, "Kauai_Elepaio", "Oahu_Amakihi", "Oahu_Elepaio", "Puaiohi")#, "Akikiki", "Anianiau", "Apapane", "Iiwi", "Kauai_Amakihi", "Kauai_Elepaio", "Oahu_Amakihi", "Oahu_Elepaio", "Puaiohi")   #"Akekee", "Akikiki", "Anianiau", "Apapane", "Iiwi", "Kauai_Amakihi", "Kauai_Elepaio", "Oahu_Amakihi", "Oahu_Elepaio", "Puaiohi"
server=1
  ...
  if (file.exists(workspace_name_out)==F | overwrite==1){
    ...
    if (baseline_or_future==1){
      ...
      for (dfd in 1:length(all_clim_data)){
        ...
        predictors_temp = stack( paste(clim_data_dir, env_var_files[1], sep=""))
        for (jj in 2:jnk0){
          predictors_temp = stack(predictors_temp, paste(clim_data_dir, env_var_files[jj], sep=""))
        }
        predictors_temp=crop(predictors_temp,  crop_raster)
        names(predictors_temp)<- var_name
        predictors_temp=stack(predictors_temp)
